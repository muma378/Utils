...
def main():
	...
	verify_parser = subparser.add_parser('inspect', help=u"检查并输出非法文本")
	verify_parser.set_defaults(builder=ob.inspect)
	...
	time_parser.add_argument('-p', '--print', dest='print_option', nargs='+', metavar='not-empty', default=('total', 'not-empty', 'valid'), 
		choices=('total', 'not-empty', 'valid'), help=u"打印类型: total - 目标文件夹内的总时长和有效时长；\nnot-empty - 含有任意标注信息即非空的总时长；\nvalid - 含有合法文本的总时长。\n默认为打印所有信息('total', 'not-empty', 'valid')")
	time_parser.add_argument('-m', '--mode', metavar='separately', default='separately', choices=('separately', 'together'), 
		help=u"统计动作：separately - 分类统计；\ntogether - 合并统计")
	time_parser.add_argument('-l', '--layer', metavar='N', default=2, type=int, help=u'被统计的层次，例如统计item[3]的时长时，该值为3。默认值为2')
