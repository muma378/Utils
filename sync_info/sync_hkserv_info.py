...
SIZE_COL_INDEX = 3 # the index of column to represent the size of dir in a project
SUMMARY_EXCEL = os.path.join(os.getcwd(), 'Summary.xltx')
...
def get_projects_list(url):
	...
	projects_list = map(lambda x: (x.text_content().strip(), url+x.get('href')), doc.xpath('//tr/td[1]/a'))
...
def retrive(queue):
	...
	try:
		while True:
			...
			data['summary'][dirname] = stat(info)
			# get project name instead of id
			data['details'][naming.get(dirname, dirname)] = info
			queue.task_done()
			counter += 1		
	except Queue.Empty:
...
def stat(info):
	summary = {'date':'', 'count':len(info), 'over450':[0, 0, 0], 'bet400and450':[0, 0, 0], 'under400':[0, 0, 0]}
...
def summarize(data):
	logger.info('writing to %s now...' % SUMMARY_EXCEL)
	wb = px.load_workbook(filename=SUMMARY_EXCEL)
	ws = wb.active
