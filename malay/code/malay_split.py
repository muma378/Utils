...
from pairwise import quick_pairwise



START_FILE_INDEX = 1
DST_DIR_NAME = 'Astro Awani Sukan Awani Wav4_'
CMD_TEMPLATE = ('truncate.exe' if sys.platform=='win32' else './truncate') + ' "{src_file}" "{dst_file}" {start} {end}'
...
def split(interval_pairs, audio_file, file_index, original_name, dst_dir=DST_DIR_NAME):
	...
	if interval_pairs[-1][0]['xmax'] > eval_wav_duration(audio_file):
		print "time annotated for {0} is beyond its real time".format(audio_file)
		return
	clip_index = 1
	for interval_index, interval_pair in enumerate(interval_pairs):
		try:
			...
			print ("unable to extract a corresponding gender in " + audio_file + " for interval index " + str(interval_index))
			continue
...
def split_pair(root_dir, file_index, pair):
	if len(pair) == 3:
		...
		audiofile, textfile = pair
		original_name = textfile
	else:
		print "split was interrupted for pair {0} has no enough elements".format(pair)
		return
	...
	if not items[1]:
		...
		try:
			...
			print("unable to match intervals in " + textfile)
			raise IOError()
...
if __name__ == '__main__':
	...
	list_txt = sys.argv[2] #503.txt or null.txt
